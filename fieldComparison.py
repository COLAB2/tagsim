...
def MidcaComLink():
    global running, searchComplete, wp_list, agent, E, det_count, agentList, t, endsim
    run = True
...
fieldMax = [(5.5,14,6),(.3*x_range,.7*y_range,14)]
field = fields[0]
measurement_time = 2
time_step=.5
...
if method==searchMethods[3] or method==searchMethods[0]:
    ...
    for i in range(len(agentList)):
        agent=agentList[i]
        pos=agent.getPos()
        wp_list[i] = search(wp_list[i], [pos[0], pos[1]])
...
f.close()
print ("Experiment output Recorded")
input('done')
