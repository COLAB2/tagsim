...
def MidcaIntegrator(agent, update):
    global running, searchComplete, wp_list, E, det_count, agentList, off, sc, searchMIDCAErgodic, mode, removeRemoraAction, start_ergodic_time
    run = True
...
def singleIntegratorErgodicControl(agent,update,scale=None,offsets=None):
    ...
    try:
        if not update:
            sock.send(str.encode(str((round(st[0], 1) - offsets[0]) / scale) + " " + str(
                (round(st[1], 1) - offsets[1]) / scale) + " " + str(t) + " " + "None "))
        else:
            sock.send(str.encode(str((round(st[0], 1) - offsets[0]) / scale) + " " + str(
                (round(st[1], 1) - offsets[1]) / scale) + " " + str(t) + " " + str(latestMeas)))
    except:
        traceback.print_exc()
        # print(e)
        run = False
